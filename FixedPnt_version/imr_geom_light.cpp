/***************************************************************************\
  File: IMR_Geom_Light.cpp
  Description: Lightsource module.
...
for (poly = 0; poly < Num_Polys; poly ++)
    ...
    if (PList[poly].Flags.LightSource || !CurrentSettings->Effects.LightingEnabled)
        {
    ...
    if (Type == IMR_LIGHT_POINT)
        ...
        for (vtx = 0; vtx < PList[poly].Num_Verts; vtx ++)
            ...
            if (Attenuation > 1.0)
                {
                PList[poly].UVI_Info[vtx].R = 1.0;
                PList[poly].UVI_Info[vtx].G = 0.0;
                PList[poly].UVI_Info[vtx].B = 0.0;
                 }           

/***************************************************************************\
  File: IMR_Table.cpp
  Description: Lookup table module.
...
// Globals:
int TablesBuilt = 0;
FIXEDPNT SinTable[IMR_DEGREECOUNT + 1], 
...
for (int Angle = 0; Angle <= IMR_DEGREECOUNT; ++ Angle)
    {
    float Degree = Angle;
    CosTable[Angle] = FloatToFixed(cos(AngToRad * Degree));
    SinTable[Angle] = FloatToFixed(sin(AngToRad * Degree));
     }

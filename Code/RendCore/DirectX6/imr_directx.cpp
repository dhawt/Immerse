...
// If this is software device, skip it:
if (!IsHardware) return D3DENUMRET_OK;

// This device'll work.  Keep it:
*(int *)UserParm = TRUE;
memcpy((void *)&Direct3DDeviceInfo.Guid, (void *)DeviceGuid, sizeof(*DeviceGuid));
...
if (Format->dwFlags & (DDPF_LUMINANCE | DDPF_BUMPLUMINANCE | DDPF_BUMPDUDV))
    return DDENUMRET_OK;
if (Format->dwFourCC != 0) return DDENUMRET_OK;
...
if (Format->dwRGBBitCount == 8)
    {
    if (*(int *)Found & 0x2) return DDENUMRET_CANCEL;
    *(int *)Found |= 0x1;   
     }
if (Format->dwRGBBitCount == 16)
    ...
    if (Format->dwRBitMask != 0x00007c00 ||
        ...
        Format->dwBBitMask != 0x0000001F) return DDENUMRET_OK;
    //if (Format->dwRBitMask != 0x00000F00 ||
    //    Format->dwGBitMask != 0x000000F0 ||
    //    Format->dwBBitMask != 0x0000000F) return DDENUMRET_OK;

    if (*(int *)Found & 0x1) return DDENUMRET_CANCEL;
    *(int *)Found |= 0x2;   
     }
return DDENUMRET_OK;

// Flag that we found our format:
//*(int *)Found = TRUE;
//return DDENUMRET_CANCEL;

//memcpy((DDPIXELFORMAT *)param, Format, sizeof(DDPIXELFORMAT)); 
 }
...
/***************************************************************************\
  Creates a 16bpp texture.
  Returns a pointer to the surface, null otherwise.
\***************************************************************************/
LPDIRECTDRAWSURFACE4 IMR_DirectXInterface::Create_16BitTexture(int Width, int Height)
{
DDSURFACEDESC2 DDSurfaceDesc;
DDSCAPS DDSCaps;
LPDIRECTDRAWSURFACE4 Surface;
...
ZeroMemory((void *)&DDSurfaceDesc.ddpfPixelFormat, sizeof(DDPIXELFORMAT));
DDSurfaceDesc.dwSize = sizeof(DDSurfaceDesc);
DDSurfaceDesc.dwFlags = DDSD_CAPS | DDSD_PITCH | //DDSD_MIPMAPCOUNT |
                        DDSD_HEIGHT | DDSD_WIDTH | DDSD_PIXELFORMAT;
DDSurfaceDesc.ddsCaps.dwCaps = DDSCAPS_TEXTURE;// | DDSCAPS_COMPLEX | DDSCAPS_MIPMAP;
//SurfaceDesc.dwMipMapCount = 4;
DDSurfaceDesc.dwWidth = Width;
DDSurfaceDesc.dwHeight = Height;
DDSurfaceDesc.lPitch = Width * 2;
DDSurfaceDesc.ddpfPixelFormat.dwSize = sizeof(DDPIXELFORMAT);
DDSurfaceDesc.ddpfPixelFormat.dwFlags = DDPF_RGB;// | DDPF_ALPHAPIXELS;
DDSurfaceDesc.ddpfPixelFormat.dwRGBBitCount = 16;
//DDSurfaceDesc.ddpfPixelFormat.dwRBitMask = 0x00000F00;
//DDSurfaceDesc.ddpfPixelFormat.dwGBitMask = 0x000000F0;
//DDSurfaceDesc.ddpfPixelFormat.dwBBitMask = 0x0000000F;
DDSurfaceDesc.ddpfPixelFormat.dwRBitMask = 0x00007c00;
DDSurfaceDesc.ddpfPixelFormat.dwGBitMask = 0x000003e0;
DDSurfaceDesc.ddpfPixelFormat.dwBBitMask = 0x0000001F;
//DDSurfaceDesc.ddpfPixelFormat.dwRGBAlphaBitMask = 0x0000F000;
...
ZeroMemory((void *)&DDSurfaceDesc.ddpfPixelFormat, sizeof(DDPIXELFORMAT));
DDSurfaceDesc.dwSize = sizeof(DDSurfaceDesc);
DDSurfaceDesc.dwFlags = DDSD_CAPS | DDSD_MIPMAPCOUNT | DDSD_PITCH |
                        DDSD_HEIGHT | DDSD_WIDTH | DDSD_PIXELFORMAT;
DDSurfaceDesc.ddsCaps.dwCaps = DDSCAPS_TEXTURE | DDSCAPS_COMPLEX | DDSCAPS_MIPMAP;
DDSurfaceDesc.dwMipMapCount = 4;
DDSurfaceDesc.dwWidth = Width;
DDSurfaceDesc.dwHeight = Height;
DDSurfaceDesc.lPitch = Width * 3;
DDSurfaceDesc.ddpfPixelFormat.dwSize = sizeof(DDPIXELFORMAT);
...
DDSurfaceDesc.ddpfPixelFormat.dwRGBAlphaBitMask = 0x00000000;
...
/***************************************************************************\
  ...
  Returns a pointer to the surface, null otherwise.
\***************************************************************************/
LPDIRECTDRAWSURFACE4 IMR_DirectXInterface::Create_Lightmap(int Width, int Height)
{
DDSURFACEDESC2 DDSurfaceDesc;
DDSCAPS        DDSCaps;
LPDIRECTDRAWSURFACE4 Surface;
...
DDSurfaceDesc.dwWidth = Width;
DDSurfaceDesc.dwHeight = Height;
DDSurfaceDesc.lPitch = Width;
DDSurfaceDesc.ddpfPixelFormat.dwSize = sizeof(DDPIXELFORMAT);
DDSurfaceDesc.ddpfPixelFormat.dwFlags = DDPF_RGB;
DDSurfaceDesc.ddpfPixelFormat.dwRGBBitCount = 8;
DDSurfaceDesc.ddpfPixelFormat.dwRBitMask = 0x000000E0;
DDSurfaceDesc.ddpfPixelFormat.dwGBitMask = 0x0000001C;
DDSurfaceDesc.ddpfPixelFormat.dwBBitMask = 0x00000003;
...
err = DirectDraw->QueryInterface(IID_IDirect3D3, (void **)&Direct3D);
if (err != S_OK)
    {
    IMR_LogMsg(__LINE__, __FILE__, "Could not query IDirect3D3 interface!");
    return IMRERR_DIRECTX;
     }

// Enumerate the devices and find one to use:
err = Direct3D->EnumDevices(&DeviceCallback, (void *)&DeviceFound);
if (err != D3D_OK)
    {
    IMR_LogMsg(__LINE__, __FILE__, "DirectX says: %s", IMR_MsgFromDXErr(err));
    return IMRERR_DIRECTX;
...
/***************************************************************************\
  Returns the linear width (pitch) of the primary surface.
\***************************************************************************/
int IMR_DirectXInterface::Get_PrimarySurfacePitch(void)
{
DDSURFACEDESC2 PrimaryDesc;

// Make sure we have a primary surface:
if (!DDPrimarySurface)
    {
    IMR_LogMsg(__LINE__, __FILE__, "No primary surface.");
    return 0;
...
ZeroMemory((void *)&PrimaryDesc, sizeof(PrimaryDesc));
PrimaryDesc.dwSize = sizeof(PrimaryDesc);
int err = DDPrimarySurface->GetSurfaceDesc(&PrimaryDesc);
if (err != DD_OK)
    {
    IMR_LogMsg(__LINE__, __FILE__, "DirectX says: %s", IMR_MsgFromDXErr(err));
    Shutdown(); return IMRERR_DIRECTX;
...
// And return the pitch:
return PrimaryDesc.lPitch;
 }
